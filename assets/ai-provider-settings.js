(() => {
  ...
  async function enhance(){
    ...
    try{
      ...
      const rows=providers.map(p=>{const c=credById.get(p.id),source=p.personal?'Personal BYOK':p.workspaceConfigured?'Workspace fallback':'—',key=c?`••••${esc(c.last4||'')}`:'—',test=c?.lastTestStatus?`<div class="muted">Test: ${esc(c.lastTestStatus)}</div>`:'';return `<tr><td><strong>${esc(p.name)}</strong>${p.custom?'<div class="muted">Custom</div>':''}</td><td>${esc(p.protocol)}</td><td>${esc(source)}</td><td>${esc(p.researchModel||p.model||'—')}</td><td>${esc(p.draftModel||p.model||'—')}</td><td>${p.capabilities?.webSearch?'Yes':'No'}</td><td>${key}${test}</td><td>${c?`<button class="mini-btn ai-key-test" data-id="${esc(p.id)}">Test</button> <button class="mini-btn ai-key-remove" data-id="${esc(p.id)}">Remove</button>`:'<span class="muted">Not personal</span>'}</td></tr>`}).join('');
      const box=document.createElement('div');box.id='universalAiProviders';box.className='card mt12';
      box.innerHTML=`<div class="page-head"><div><h2>AI Providers · Personal BYOK</h2><p>每位同事使用自己的 API Key。Key 仅保存为服务端加密密文，浏览器不会保存或回显完整 Secret。</p></div><span class="pill ${connected.length?'ok':'warn'}">${connected.length} available</span></div>
      ${state.masterKeyReady?'':'<div class="notice bad"><strong>管理员一次性配置：</strong> Cloudflare Secret <code>AI_CREDENTIALS_MASTER_KEY</code> 尚未设置。设置后，同事才可以从网页保存个人 Key。</div>'}
      <div class="table-wrap mt12"><table class="table"><thead><tr><th>Provider</th><th>Protocol</th><th>Source</th><th>Research Model</th><th>Draft Model</th><th>Web Search</th><th>Personal Key</th><th>Action</th></tr></thead><tbody>${rows||'<tr><td colspan="8"><div class="empty">No providers</div></td></tr>'}</tbody></table></div>
      ...
      <div class="form-grid three mt12">
        ...
        <div class="field byok-custom hidden"><label><input id="byokWebSearch" type="checkbox"> Provider has native Web Search</label></div>
      </div>
      ...
      <div class="notice info mt12">个人 Key 优先于 Workspace Key。同事 A 与同事 B 的 Key、默认模型和 Provider 互相独立。删除个人 Key 后，只有在启用 Workspace fallback 时才会使用管理员配置的团队 Key。</div>`;
      content.appendChild(box);
      const providerSel=$('#byokProvider',box),name=$('#byokName',box),model=$('#byokModel',box),customFields=$$('.byok-custom',box);
      const syncForm=()=>{const id=providerSel.value,isCustom=id==='__custom__';customFields.forEach(x=>x.classList.toggle('hidden',!isCustom));if(isCustom){name.value='';model.value='';return}const p=providers.find(x=>x.id===id);name.value=p?.name||id;model.value=p?.draftModel||p?.model||p?.researchModel||''};providerSel.onchange=syncForm;syncForm();
      $('#byokSave',box).onclick=async()=>{const isCustom=providerSel.value==='__custom__',providerId=isCustom?$('#byokCustomId',box).value:providerSel.value,apiKey=$('#byokApiKey',box).value.trim();if(!apiKey){toast('请输入 API Key',true);return}const payload={providerId,providerName:name.value,model:model.value,apiKey};if(isCustom){payload.protocol=$('#byokProtocol',box).value;payload.baseUrl=$('#byokBaseUrl',box).value;payload.capabilities={webSearch:$('#byokWebSearch',box).checked}}const b=$('#byokSave',box);b.disabled=true;b.textContent='Saving…';try{await api('/api/ai/credentials',{method:'POST',body:JSON.stringify(payload)});$('#byokApiKey',box).value='';toast('个人 API Key 已加密保存');setTimeout(()=>location.reload(),500)}catch(e){toast(e.data?.detail||e.data?.error||e.message,true);b.disabled=false;b.textContent='Save My Key'}};
