const EXPECTED_TABLES = [
  'schema_meta','tenants','roles','users','user_roles','user_credentials','sessions','auth_attempts','settings','companies','contacts','leads','products','skus','sku_packages','opportunities','tasks','quotes','quote_items','load_plans','load_plan_items','automations','automation_runs','documents','orders','order_items','document_versions','shipments','outreach_drafts','audit_logs'
];
const COUNT_TABLES = ['tenants','users','sessions','companies','contacts','leads','products','skus','sku_packages','opportunities','tasks','quotes','quote_items','orders','order_items','documents','document_versions','shipments','outreach_drafts','load_plans','load_plan_items','automations','automation_runs','audit_logs'];
...
export async function onRequestGet(context) {
  ...
  try{
    ...
    return Response.json({ok:true,bound:true,binding:'DB',schemaReady:missingTables.length===0&&String(schemaVersion)==='4',schemaVersion,expectedSchemaVersion:'4',tables,missingTables,counts},{headers:{'Cache-Control':'no-store'}});
