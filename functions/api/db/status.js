const EXPECTED_TABLES = [
  'schema_meta','tenants','roles','users','user_roles','user_credentials','sessions','auth_attempts','settings','companies','contacts','leads','products','skus','sku_packages','opportunities','tasks','quotes','quote_items','load_plans','load_plan_items','load_plan_unloaded_items','automations','automation_runs','documents','orders','order_items','document_versions','document_discrepancies','document_custody','shipments','shipment_events','shipment_legs','letters_of_credit','lc_required_documents','lc_revisions','outreach_drafts','ai_provider_credentials','ai_user_preferences','audit_logs'
];
const COUNT_TABLES = ['tenants','users','sessions','companies','contacts','leads','products','skus','sku_packages','opportunities','tasks','quotes','quote_items','orders','order_items','documents','document_versions','document_discrepancies','document_custody','shipments','shipment_events','shipment_legs','letters_of_credit','lc_required_documents','lc_revisions','outreach_drafts','ai_provider_credentials','ai_user_preferences','load_plans','load_plan_items','load_plan_unloaded_items','automations','automation_runs','audit_logs'];
...
export async function onRequestGet(context) {
  ...
  try{
    ...
    return Response.json({ok:true,bound:true,binding:'DB',schemaReady:missingTables.length===0&&String(schemaVersion)==='7',schemaVersion,expectedSchemaVersion:'7',tables,missingTables,counts},{headers:{'Cache-Control':'no-store'}});
