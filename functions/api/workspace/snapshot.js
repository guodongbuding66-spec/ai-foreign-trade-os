...
export async function onRequestGet(context) {
  ...
  try {
    const [contactsResult,productsResult,skusResult,packagesResult,quotesResult,quoteItemsResult,opportunitiesResult,tasksResult,automationsResult,loadPlansResult] = await context.env.DB.batch([
    ...
    for (const item of quoteItemsResult.results || []) { const list=quoteItemsByQuote.get(item.quote_id)||[]; list.push(item); quoteItemsByQuote.set(item.quote_id,list); }
    ...
    return json({
      ...
      opportunities:(opportunitiesResult.results||[]).map(normalizeOpportunity),
      tasks:(tasksResult.results||[]).map(normalizeTask),
      automations:(automationsResult.results||[]).map(normalizeAutomation),
      loadPlans:(loadPlansResult.results||[]).map(normalizeLoadPlan)
    });
  } catch (error) {
