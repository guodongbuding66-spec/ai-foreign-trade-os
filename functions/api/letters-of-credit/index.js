...
export async function onRequestPost(context){
  ...
  const data=normalizeLCBody(body),requestedId=String(body?.id||'').trim();if(!data.orderId)return json({ok:false,error:'order_id_required'},400);if(!data.lcNo)return json({ok:false,error:'lc_no_required'},400);
  try{
    ...
    const existing=requestedId?await context.env.DB.prepare('SELECT * FROM letters_of_credit WHERE tenant_id=? AND id=? LIMIT 1').bind(auth.tenant.id,requestedId).first():await context.env.DB.prepare('SELECT * FROM letters_of_credit WHERE tenant_id=? AND lc_no=? LIMIT 1').bind(auth.tenant.id,data.lcNo).first();
    if(requestedId&&!existing)return json({ok:false,error:'lc_not_found'},404);
    ...
    const statements=[];
    if(existing)statements.push(context.env.DB.prepare(`UPDATE letters_of_credit SET order_id=?,document_id=?,lc_no=?,applicant_name=?,beneficiary_name=?,currency=?,amount=?,issue_date=?,expiry_date=?,presentation_place=?,latest_shipment_date=?,partial_shipment=?,transshipment=?,status=?,raw_terms_json=?,updated_at=? WHERE id=? AND tenant_id=?`).bind(data.orderId,data.documentId,data.lcNo,data.applicantName,data.beneficiaryName,data.currency,data.amount,data.issueDate||null,data.expiryDate||null,data.presentationPlace,data.latestShipmentDate||null,data.partialShipment,data.transshipment,data.status,JSON.stringify(rawTerms),now,id,auth.tenant.id));
