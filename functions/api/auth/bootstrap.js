import {
  assertSameOrigin, audit, constantTimeEqual, createSession, json,
...
export async function onRequestPost(context) {
  try {
    ...
    try {
      await context.env.DB.batch([
        ...
        context.env.DB.prepare(`
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?)
        `).bind(userId, credential.hash, credential.salt, credential.iterations, credential.algorithm, now, now),
      ...
      return json({
        ok: false,
        error: duplicateBootstrap ? 'bootstrap_disabled' : 'bootstrap_failed',
        detail: duplicateBootstrap ? undefined : String(error?.message || 'database_write_failed').slice(0, 180)
      }, duplicateBootstrap ? 409 : 500);
    }
    ...
    return json({ ok: false, error: 'bootstrap_runtime_error', detail: String(error?.message || error).slice(0, 180) }, 500);
  }
}
