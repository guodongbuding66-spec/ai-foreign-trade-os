...
export async function onRequestPost(context){
  ...
  try{
    const result=await runStructuredAI(userAI.env,{task:'research',providerId:String(body?.providerId||''),webSearch:true,maxTokens:2600,schema:researchSchema,system:'You are a rigorous B2B customer research analyst for international trade. Evidence quality is more important than completeness.',prompt});
    const research=result.data,sources=result.sources||[],now=new Date().toISOString(),credentialScope=userAI.personalIds.has(result.provider.id)?'personal':'workspace';
    if(lead){let oldEvidence=[];try{oldEvidence=JSON.parse(lead.source_evidence_json||'[]')}catch{}const existingUrls=new Set(oldEvidence.map(x=>x?.url).filter(Boolean));const sourceEvidence=sources.filter(x=>!existingUrls.has(x.url)).map(x=>({type:'web',provider:result.provider.id,title:x.title,url:x.url,researchedAt:now}));const researchEvidence={type:'ai_research',provider:result.provider.id,model:result.model,credentialScope,researchedAt:now,fitScore:research.fitScore,confidence:research.confidence,summary:String(research.summary||'').slice(0,1200)};const merged=[...oldEvidence.filter(x=>x?.type!=='ai_research'),...sourceEvidence,researchEvidence].slice(-30);await context.env.DB.prepare('UPDATE leads SET score=?, source_evidence_json=?, updated_at=? WHERE id=? AND tenant_id=?').bind(Math.max(0,Math.min(100,Number(research.fitScore||0))),JSON.stringify(merged),now,lead.id,tenantId).run();}
    await audit(context,{tenantId,userId:auth.user.id},'ai.research',lead?'lead':'company',lead?.id||resolvedCompanyId||null,null,{provider:result.provider.id,model:result.model,credentialScope,fitScore:research.fitScore,confidence:research.confidence,sourceCount:sources.length});
    return json({ok:true,provider:result.provider.id,providerName:result.provider.name,credentialScope,model:result.model,research,sources,usage:result.usage||null});
