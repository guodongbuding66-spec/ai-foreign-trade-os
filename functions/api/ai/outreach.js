...
async function enforceDailyLimit(context,auth){const limit=Math.max(1,Math.min(1000,Number(context.env.AI_DRAFT_DAILY_LIMIT||100)));const since=new Date(Date.now()-86400000).toISOString();const row=await context.env.DB.prepare("SELECT COUNT(*) AS count FROM audit_logs WHERE tenant_id=? AND user_id=? AND action='ai.outreach.generate' AND created_at>=?").bind(auth.tenant.id,auth.user.id,since).first();if(Number(row?.count||0)>=limit)return json({ok:false,error:'ai_daily_limit_reached',limit},429);return null;}
...
export async function onRequestPost(context){
  ...
  const research=body?.research&&typeof body.research==='object'?body.research:null,selectedProduct=product?{name:product.name,category:product.category,series:product.series,material:product.material,market:product.market}:null;
