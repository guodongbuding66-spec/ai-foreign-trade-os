...
function docTypeFromLine(line){const s=line.toLowerCase();if(/commercial invoice|signed invoice/.test(s))return'CI';if(/packing list|packing\/weight list/.test(s))return'PL';if(/bill of lading|ocean b\/l|marine bill/.test(s))return'BL';if(/air waybill|airway bill|awb/.test(s))return'AWB';if(/sea waybill/.test(s))return'SEA_WAYBILL';if(/insurance policy|insurance certificate/.test(s))return'INSURANCE';if(/certificate of origin|origin certificate/.test(s))return'COO';if(/inspection certificate|certificate of inspection/.test(s))return'INSPECTION';if(/beneficiary.*certificate/.test(s))return'BENEFICIARY_CERT';return'OTHER'}
